define(['jquery', 'core/str'], function($, Str) {
    var PdfCounterMonitor = {
        ...
        evaluatePendingPdfs: function() {
            var self = this;
            // Chama ajax_eval_pdf.php atÃ© nÃ£o restar pendente
            function evalNext() {
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.done) {
                            // Todos avaliados, atualiza dashboard uma vez e pÃ¡ra o loop
                            setTimeout(function() {
                                self.refreshDashboard();
                            }, 500);
                        } else {
                            // Avaliou um, atualiza dashboard e chama prÃ³ximo
                            setTimeout(function() {
                                self.refreshDashboard();
        ...
        updateDashboardUI: function(data) {
            ...
            // Se o backend jÃ¡ enviar o nÃºmero de PDFs pendentes, usar esse valor
            ...
            if (pendingMsg) {
                if (pendingCount > 0) {
                    ...
                    Str.get_string('pendingmsg_analyzing', 'block_pdfcounter', pendingCount)
                        .then(function(msg) {
                            pendingMsg.innerHTML = 'âš ï¸ ' + msg;
                            pendingMsg.style.display = '';
                        })
                        .catch(function() {
                            pendingMsg.innerHTML = 'âš ï¸ ' + pendingCount + ' PDF(s)';
                            pendingMsg.style.display = '';
                        });
                } else {
            ...
            if (totalElement.length) {
                Str.get_string('totalpdfs', 'block_pdfcounter', data.totalPdfs)
                    ...
                    .catch(function() {
                        // Fallback: mostra sÃ³ o nÃºmero se nÃ£o conseguir ir buscar a string
        ...
        updatePdfIssues: function(pdfIssues) {
            ...
            pdfIssues.sort(function(a, b) {
                ...
                var rowPromises = pdfIssues.map(function(pdf) {
                    ...
                    // Preferir o nome "amigÃ¡vel" vindo do Moodle (tÃ­tulo da atividade)
                    // e recuar para o nome de ficheiro caso nÃ£o exista.
                    var label = pdf.display_name || pdf.filename;
                    // Garante que o utilizador vÃª claramente que Ã© um PDF,
                    // adicionando ".pdf" se ainda nÃ£o estiver presente.
                    var labelWithExt = label;
                    if (!/\.pdf$/i.test(labelWithExt)) {
                        labelWithExt = labelWithExt + '.pdf';
                    }
                    ...
                    return Promise.all([
                        ...
                        rowHtml += '<div class="div1" style="grid-area:1/1/2/2; align-self:start; font-size:1em;">' + labelWithExt + '</div>';
                        ...
                        // Fallback para inglÃªs caso haja erro a obter strings
                        var fallback = '';
                        ...
                        fallback += '<div class="div1" style="grid-area:1/1/2/2; align-self:start; font-size:1em;">' + labelWithExt + '</div>';
