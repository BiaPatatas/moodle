...
// Validate course ID
if ($courseid <= 0) {
    block_pdfcounter_debug_log('download_report.php: courseid inválido', [
        'courseid' => $courseid,
        'filename' => $filename,
    ], 'download_report.log');
...
if (!$DB->record_exists('course', array('id' => $courseid))) {
    block_pdfcounter_debug_log('download_report.php: curso não encontrado', [
        'courseid' => $courseid,
        'filename' => $filename,
    ], 'download_report.log');
...
if (!$pdfrecord) {
    $filehash_param = isset($filehash) ? $filehash : null;
    block_pdfcounter_debug_log('download_report.php: relatório não encontrado', [
        'courseid' => $courseid,
        'filename' => $filename,
        'filehash_param' => $filehash_param,
    ], 'download_report.log');
