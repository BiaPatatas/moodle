...
try {
    ...
    // Debug: log found files (apenas em caso de erro, ver abaixo)
    $filenames = [];
            ...
            if (!$result) {
                if (function_exists('pdf_accessibility_log_error')) {
                    pdf_accessibility_log_error('preview.php: Python analysis returned invalid JSON', [
                        ...
                        'output' => isset($output) ? (string)$output : null,
                    ]);
                }
                continue;
            }
    ...
    if (function_exists('pdf_accessibility_log_error')) {
        pdf_accessibility_log_error('preview.php: no PDF analyzed successfully', [
            'draftid' => $draftid,
            'courseid' => $courseid,
            'filenames' => $filenames,
        ]);
    ...
    echo json_encode(['status' => 'error', 'message' => 'Unexpected error while analyzing PDF']);
    exit;
}